...
class PosOrientWithCovTyped2DataFrame(CSV2DataFrame):
    ...
    def DataFrame_to_TPQCov(data_frame):
        ...
        _vec, p_vec, q_vec, P_vec_p, P_vec_q = PosOrientWithCov2DataFrame.DataFrame_to_TPQCov(data_frame)
        if version_info[0] < 3:
    ...
    @staticmethod
    def TPQCov_to_DataFrame(t_vec, p_vec, q_vec, P_vec_p, P_vec_q, est_err_type_vec, err_rep_vec):
        t_rows, t_cols = t_vec.shape  # does not work in Python 3
        assert (len(est_err_type_vec) == t_rows)
        assert (len(err_rep_vec) == t_rows)

        df1 = PosOrientWithCov2DataFrame.TPQCov_to_DataFrame(t_vec, p_vec, q_vec, P_vec_p, P_vec_q)
